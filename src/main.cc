/**
  ...
  This is an entry point to the application source.
  We use C++11 inside, it is possible in CUDA 6.5 and newer with -std=c++11
  switch.
...
int main(int argc, char *argv[]) {
    ...
    params.ants_count_ = (uint32_t)options.get_option("--ants", (int)dimension);
    ...
    auto nn_lists = init_nn_lists(dist_matrix);
